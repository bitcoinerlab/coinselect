...
export async function getVsizeFixtureData() {
  await ready;
  ...
  const trInternalKey = keyExpressionBIP32({
    masterNode,
    originPath: "/0'",
    keyPath: '/2'
  });
  const trLeafKeyA = keyExpressionBIP32({
    masterNode,
    originPath: "/0'",
    keyPath: '/3'
  });
  const trLeafKeyB = keyExpressionBIP32({
    masterNode,
    originPath: "/0'",
    keyPath: '/4'
  });
  const trLeafKeyC = keyExpressionBIP32({
    masterNode,
    originPath: "/0'",
    keyPath: '/5'
  });
  const trLeafPubKeyA = parseKeyExpression({
    keyExpression: trLeafKeyA,
    network
  }).pubkey;
  if (!trLeafPubKeyA) throw new Error('Cound not compute trLeafPubKeyA');
  const trLeafPubKeyB = parseKeyExpression({
    keyExpression: trLeafKeyB,
    network
  }).pubkey;
  if (!trLeafPubKeyB) throw new Error('Cound not compute trLeafPubKeyB');
  ...
  const trScriptPathComplexLeaf = `sortedmulti_a(2,${trLeafKeyA},${trLeafKeyB},${trLeafKeyC})`;
  ...
  const outputsData: OutputsDataType = {
    ...
    trScriptPathComplex: {
      descriptor: `tr(${trInternalKey},{pk(${delayedKey}),{pk(${instantKey}),${trScriptPathComplexLeaf}}})`,
      // Force the complex script leaf selection for tapscript vsize fixtures.
      signersPubKeys: [trLeafPubKeyA, trLeafPubKeyB]
    },
    shWpkh: {
  ...
  const inputCombinationKeys = [
    ...
    'wshInstant',
    'shDelayed',
    // 'shInstant', // Disabled for input combinations to limit exponential fixture growth.
    'shWshDelayed',
    'shWshInstant'
  ];

  const inputCombinations = getAllCombinations(
    outputsData,
    inputCombinationKeys
  );
  const outputKeys = [
    'pkh',
    'wpkh',
  ...
  return { network, masterNode, transactions };
}
